...
# Siqnal öncəsi xəbərdarlıq
async def send_pre_signal_warning():
    pre_signal_message = "🚨 Siqnallar Üçün Hazır Olun! 2 Dəqiqə Qalıb!"
    try:
        await bot.send_message(chat_id=CHANNEL_ID, text=pre_signal_message)
        print("Siqnal öncəsi xəbərdarlıq göndərildi! ✅")
    except Exception as e:
        print(f"Siqnal öncəsi xəbərdarlıq göndərilərkən xəta baş verdi: {e}")

# Növbəti tur mesajı
async def send_next_round_message():
    next_round_message = "🎰Növbəti Tur 8-Dəqiqə Sonra!"
    try:
        await bot.send_message(chat_id=CHANNEL_ID, text=next_round_message)
        print("Növbəti tur mesajı göndərildi! ✅")
    except Exception as e:
        print(f"Növbəti tur mesajı göndərilərkən xəta baş verdi: {e}")

# Qazanc mesajı göndərmək
...
# Mərc mesajı göndərmək
async def send_bet_message(random_time):
    await send_pre_signal_warning()  # Siqnal öncəsi xəbərdarlıq
    await asyncio.sleep(2 * 60)  # 2 dəqiqə gözləyirik
    message = (
        f"🎯 Bu günün mərci 🎯\n\n"
    ...
    await asyncio.sleep(8 * 60)  # 8 dəqiqə gözləyirik
    await send_next_round_message()  # Növbəti tur mesajı

# Mərc mesajlarının avtomatik göndərilməsi
...
async def main():
    ...
    await asyncio.gather(
        ...
        send_earnings_message()  # Qazanc mesajlarını göndərir
    )
